...
class UserRepositoryTest {
				...
				fun `test updateUser with a valid user url, should return true`() = runTest {
								...
								verify(exactly = 1) {
												db.collection(com.joohnq.domain.constants.FirebaseConstants.FIREBASE_USER).document(any<String>()).set(any<User>())
								}
				...
				fun `test updateUser with a valid user url, but with a error updating, should return an exception`() =
								runTest {
												val exception = Exception(com.joohnq.domain.constants.Constants.TEST_SOME_ERROR)
												val userMockk = mockk<User>(relaxed = true)
												auth = mockk(relaxed = true) { every { currentUser?.uid } returns userId }
												db = mockk(relaxed = true)
												userRepository = UserRepositoryImpl(auth, db, storage)
				...
				fun `test fetchUser with a valid user url, should return true`() = runTest {
								...
								every { db.collection(com.joohnq.domain.constants.FirebaseConstants.FIREBASE_USER).document(any()).get() } returns task

								val user = userRepository.getUser()

								verify(exactly = 1) {
												db.collection(com.joohnq.domain.constants.FirebaseConstants.FIREBASE_USER).document(any<String>()).get()
								}
				...
				fun `test uploadUserImage with a valid user uri, should return true`() = runTest {
								...
								every {
												storage.getReference(com.joohnq.domain.constants.FirebaseConstants.FIREBASE_USERS)
																.child(com.joohnq.domain.constants.FirebaseConstants.FIREBASE_PHOTOS)
																.child(any<String>())
																.putFile(uri)
								} returns uploadTask
